    ...
    for idx in pbar:
        ...
        for step_index in range(args.n_step_d):
            ...
            if args.lambda_rec_d > 0 and not args.decouple_d:  # Do not train D on x_rec if decouple_d
                ...
                rec_img, _ = generator(
                    [latent_real], input_is_latent=input_is_latent)
                if args.augment:
            ...
            d_loss = d_loss_real + d_loss_fake * \
                args.lambda_fake_d + d_loss_rec * args.lambda_rec_d
            loss_dict["d"] = d_loss
        ...
        for step_index in range(args.n_step_e):
            ...
            if args.lambda_rec_w > 0:
                ...
                noise = mixing_noise(
                    args.batch, args.latent, mixing_prob, device)
                fake_img, latent_fake = generator(
                    noise, return_latents=True, detach_style=True)
            ...
            if i % args.log_every == 0:
                with torch.no_grad():
                    ...
                    ref_vgg_loss = torch.mean(
                        (vggnet(sample_x) - vggnet(rec_real)) ** 2) if vggnet is not None else 0
                    # Fixed fake samples and reconstructions
                    sample_gz, _ = g_ema([sample_z])
                    latent_gz, _ = e_ema(sample_gz)
                    rec_fake, _ = g_ema(
                        [latent_gz], input_is_latent=input_is_latent)
            ...
            if args.eval_every > 0 and i % args.eval_every == 0:
                with torch.no_grad():
                    ...
                    if 'fid_sample' in args.which_metric:
                        ...
                        fid_sa = calc_fid(
                            sample_mean, sample_cov, real_mean, real_cov)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str,
                        help="experiment name", default='default_exp')
    parser.add_argument("--log_root", type=str,
                        help="where to save training logs", default='logs')
    parser.add_argument("--log_every", type=int, default=100,
                        help="save samples every # iters")
    parser.add_argument("--save_every", type=int, default=1000,
                        help="save checkpoints every # iters")
    parser.add_argument("--save_latest_every", type=int,
                        default=200, help="save latest checkpoints every # iters")
    parser.add_argument(
    ...
    parser.add_argument("--use_wscale", action='store_true',
    ...
    parser.add_argument("--lambda_pix", type=float,
                        default=1.0, help="recon loss on pixel (x)")
    parser.add_argument("--lambda_fake_d", type=float, default=1.0)
    parser.add_argument("--lambda_rec_d", type=float,
                        default=1.0, help="d1, recon of real image")
    parser.add_argument("--lambda_fake_g", type=float, default=1.0)
    parser.add_argument("--lambda_rec_w", type=float,
                        default=0, help="recon sampled w")
    ...
    parser.add_argument("--n_sample_fid", type=int, default=50000,
    ...
    parser.add_argument("--d2_ckpt", type=str, default=None,
                        help="path to the checkpoint of discriminator2")
    parser.add_argument("--train_from_scratch", action='store_true')
    parser.add_argument("--limit_train_batches", type=float, default=1)
    parser.add_argument("--g_decay", type=float,
                        default=None, help="g decay factor")
    parser.add_argument("--n_mlp_g", type=int, default=8)
    parser.add_argument("--ema_kimg", type=int, default=10,
                        help="Half-life of the exponential moving average (EMA) of generator weights.")
    parser.add_argument("--ema_rampup", type=float,
                        default=None, help="EMA ramp-up coefficient.")
    parser.add_argument("--no_ema_e", action='store_true')
    parser.add_argument("--no_ema_g", action='store_true')
    parser.add_argument("--which_metric", type=str, nargs='*',
                        default=['fid_sample', 'fid_sample_recon', 'fid_recon'])
    ...
    parser.add_argument("--latent_space", type=str,
                        default='w', help="latent space (w | p | pn | z)")
    parser.add_argument("--lambda_rec_w_extra", type=float,
                        default=0, help="recon sampled w")
    ...
    vgg_ckpt = torch.load(
        args.vgg_ckpt, map_location=lambda storage, loc: storage)
    ...
    if args.resume:
        ...
        if args.e_ckpt is not None:
            ...
            e_ckpt = torch.load(
                args.e_ckpt, map_location=lambda storage, loc: storage)
        ...
        if args.g_ckpt is not None:
            ...
            g_ckpt = torch.load(
                args.g_ckpt, map_location=lambda storage, loc: storage)
        ...
        if args.d_ckpt is not None:
            ...
            d_ckpt = torch.load(
                args.d_ckpt, map_location=lambda storage, loc: storage)
        ...
        if args.d2_ckpt is not None:
            ...
            d2_ckpt = torch.load(
                args.d2_ckpt, map_location=lambda storage, loc: storage)
    ...
    if args.limit_train_batches < 1:
        indices = torch.randperm(len(dataset))[:int(
            args.limit_train_batches * len(dataset))]
    ...
    loader = data.DataLoader(
        ...
        sampler=data_sampler(dataset1, shuffle=True,
    ...
    if args.eval_every > 0:
        ...
        loader2 = data.DataLoader(
            dataset2, batch_size=64, num_workers=4, shuffle=False)
